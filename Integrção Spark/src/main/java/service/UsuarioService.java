...
public class UsuarioService {
	...
	private final int FORM_ORDERBY_SEXO = 2;
	...
	public void makeForm() {
		...
		makeForm(FORM_INSERT, new Usuario(), FORM_ORDERBY_CODIGO);
	}
	...
	public void makeForm(int tipo, Usuario usuario, int orderBy) {
		...
		if(tipo == FORM_INSERT || tipo == FORM_UPDATE) {
			...
			umUsuario += "\t\t</tr>";
			umUsuario += "\t\t<tr>";
			umUsuario += "\t\t\t<td>&nbsp;Sexo: <input class=\"input--register\" type=\"text\" name=\"sexo\" value=\""+ sexo +"\"></td>";
		...
		} else if (orderBy == FORM_ORDERBY_LOGIN){      usuarios = usuarioDAO.getOrderByLogin();											usuarios = usuarioDAO.get();
		...
		for (Usuario u : usuarios) {
			...
			list += "\n<tr bgcolor=\""+ bgcolor +"\">\n" + 
            		  ...
            		  "\t<td align=\"center\" valign=\"middle\"><a href=\"javascript:confirmarDeleteUsuario('" + u.getCodigo() + "', '" + u.getSexo() + "', '" + u.getLogin() + "');\"><img src=\"/image/delete.png\" width=\"20\" height=\"20\"/></a></td>\n" +
            		  "</tr>\n";
		}
		list += "</table>";		
		form = form.replaceFirst("<LISTAR-USUARIOS>", list);				
	}
	...
	public Object insert(Request request, Response response) {
        ...
        char sexo = request.queryParams("sexo");
		String login = request.queryParams("login");
		
		String resp = "";
		
		Usuario usuario = new Usuario(-1, nome, sexo, login);
	...
	public Object update(Request request, Response response) {
        ...
        if (usuario != null) {
        	...
        	usuario.setSexo(Float.parseFloat(request.queryParams("sexo")));
        	usuario.setSenha(request.queryParams("senha"));
        	usuarioDAO.update(usuario);
        	response.status(200); // success
            resp = "Usuario (ID " + usuario.getCodigo() + ") atualizado!";
        } else {
            response.status(404); // 404 Not found
            resp = "Usuario (ID \" + usuario.getCodigo() + \") não encontrado!";
        }
		makeForm();
		return form.replaceFirst("<input type=\"hidden\" id=\"msg\" name=\"msg\" value=\"\">", "<input type=\"hidden\" id=\"msg\" name=\"msg\" value=\""+ resp +"\">");
	}
